...
Conta::Conta(ClientePF c, Agencia a, std::string n) : titular(c), agencia(a), numeroDaConta(n), saldo(0)
{
	quantidadeContas++;
}

Conta::Conta(ClientePF c, Agencia a, std::string n, double valor) : titular(c), agencia(a), numeroDaConta(n), saldo(valor)
{
	quantidadeContas++;
}
